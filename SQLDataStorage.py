...
class FolderStorage(DataStorage):
    def write(self, path:str, name:str, UID:int):
        connect = sqlite3.connect(str(self.file))
        cursor = connect.cursor()
        data = f"""INSERT INTO {self.table} (path, name, UID, UUID) VALUES 
        ("{path}", "{name}", "{UID}", "{UUID(bin(f"{path}{name}{UID}"))}")
        """

        print(UUID(f"{path}{name}{UID}"))
        connect.commit()
        connect.close()
...
if __name__ == "__main__":
    FolderStorage("./static/_data/mydatabase.db", "folders").write("/home/", "programs", 0)